...
keyboard = {
    "one_time" : False,
    "buttons" : [
        [get_but('Замена', 'secondary'), get_but('Расписание', 'primary')],
        ...
        for event in longpoll.listen():
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                if event.from_chat:
                    ...
                    if msg == 'расписание':
                        sender(id, ''.join(ps.parse(droup_for_db, 'Расписание', True)))
                        sender(id, 'Внимание!! Бот находится на ранней стадии разработки, не стоит ему полностью доверять!')
                    elif msg == 'расписание -замена':
                        sender(id, ''.join(ps.parse(droup_for_db, 'Расписание', False)))
                        sender(id, 'Внимание!! Бот находится на ранней стадии разработки, не стоит ему полностью доверять!')
                    elif msg == 'замена':
                        print('Отреагировал')
                        sender(id, ''.join(ps.parse(droup_for_db, 'Замена', False)))
                    elif msg == '/debug':
